...
def browserbase_google_image_urls_validated(query: str, n: int = 5, max_candidates: int = 30) -> str:
    ...
    approved = []
    rejected = []
    print(f"[RAG] validating candidates={len(candidates)} target={n}")
    ...
    for url in candidates:
        ...
        if verdict.get("relevant"):
            ...
            print(
                f"[RAG][ACCEPTED] url={url} "
                f"subject={entry['identified_subject']} "
                f"confidence={entry['confidence']}"
            )
        else:
            rejected.append(entry)
            print(
                f"[RAG][REJECTED] url={url} "
                f"subject={entry['identified_subject']} "
                f"confidence={entry['confidence']} "
                f"reason={entry['reason']}"
            )

    print(f"[RAG] validation complete approved={len(approved)} rejected={len(rejected)}")
...
SYSTEM_PROMPT = (
    ...
    "If the user asks for photos, images, image URLs, or visual references, use optimized_image_and_maps_tool first before giving any conclusion. "
    ...
    "Do not say that you could not find images, maps, or references unless you have already called the relevant tool and inspected its result. "
    "If a tool may help, respond with a tool call first and no explanatory text. "
...
human_input = os.environ.get("RAG_INPUT") or (sys.argv[1] if len(sys.argv) > 1 else "")
if not human_input:
...
while True:
    ...
    for chunk in model_with_tools.stream(messages):
        if chunk.content:
            buffered_text.append(_chunk_content_to_text(chunk.content))
